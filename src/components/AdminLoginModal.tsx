...
export const AdminLoginModal: React.FC<AdminLoginModalProps> = ({
  ...
  // Initialize Google Identity Services (GSI) with user's Google Client ID
  const GOOGLE_CLIENT_ID = '205457229020-e3l3qab23hqt2omel5mq69n3ei7sbm5m.apps.googleusercontent.com';
          ...
          (window as any).google.accounts.id.initialize({
            ...
            callback: async (response: any) => {
              try {
                ...
                setLoading(true);
                const res = await verifyAndLoginAdminEmail(userEmail);
                setLoading(false);

                if (res.success) {
                  setSuccessMsg(`أهلاً بك ${userName}! تم التحقق من حساب Google المعتمد.`);
                  setTimeout(() => {
                    onLoginSuccess();
                    onClose();
                  }, 600);
                ...
                setLoading(false);
                setLocalError('فشل التحقق من جلسة حساب Google');
              }
            }
          });
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-3 sm:p-4 bg-stone-950/75 backdrop-blur-xs">
      ...
      >
        ...
        <div className="p-6 space-y-5">
          ...
          {/* Primary Action: Official Google Identity Services & Supabase OAuth */}
          <div className="space-y-3">
            {/* Google Identity Services official native button container */}
            <div className="flex justify-center w-full min-h-[44px]">
              <div id="gsi-official-button" className="w-full flex justify-center"></div>
            </div>
            ...
            <button
              ...
              disabled={loading}
              className="w-full py-3 px-4 rounded-2xl font-bold text-xs bg-white hover:bg-stone-50 text-stone-800 border border-stone-300 hover:border-stone-400 shadow-2xs transition-all flex items-center justify-center gap-2.5 active:scale-98 cursor-pointer"
            >
              ...
              <span>{loading ? 'جاري الاتصال بـ Supabase Google Auth...' : 'تسجيل الدخول عبر Supabase OAuth (نافذة مستقلة)'}</span>
            </button>
          </div>
